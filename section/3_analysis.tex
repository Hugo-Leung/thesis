...
\end{equation}
where $P_L$ is the longitudinal momentum of the virtual photon in the hadron-hadron center of mass frame.
At SeaQuest, we defined $x_F = \frac{2P_L}{\sqrt{s}\left[1-M^2_\gamma/s\right]}$, therefore
\begin{equation}
...
other components are allowed to float. The live proton is chosen for the normalization here, as the
...
To solve for the maximum, \cref{eq:TF_likelihood} is differentiated with respect to $p_j$ 
...
\begin{equation}
	\sum_i A_{ji}=\sum_i a_{ji}.
\end{equation}
These means the estimate of $A_{ji}$ for some source will change the shape of the distribution
...
In the case of Monte Carlo, $N=\sum weight$ is the weighted integral of the

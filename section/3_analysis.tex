...
they are less likely to be park of a muon track. 

\paragraph{In-time cut}
During data taking, the TDC time window is deliberately set to be larger than the in-time
window. This is done to avoid data loss in case of timing drift. During the offline
processing, only in-time hits will be kept. This cut is applied to all detectors.
...
There are three types of cluster hits: electronic noise, cell-edge hits and delta-rays.
...
the RF vucket periodicity, the hodoscopes are used for triggering. The hodoscope hits
...
that fall behind a fired hodoscope paddle, which is part of a valid trigger road, will be retained.

\paragraph{Sagitta ratios}
\pdfcomment{Why do we expect the ratio to be constant?}
...
$X'$ planes, a hit pairs is formed by locating a hit on each plane within one half cell spacing.
...
should also be pointing at the target (or beam dump), this done by requiring the x and y slope to be
less than $(0.15,0.1)$.

Once all tracklets are constructed separately in St2.~ and St.~3. We try to connect a tracklet
...
The mass templates for physics sources such as Drell-Yan process and charmonium decays are
generated by analyzing the Monte Carlo simulation. 
...
two separate interactions, and there it is expected proportional to the intensity squared.
...
Given a central PDF member $S_0$ and $2N_{par}$ eigenvector PDF member $S^\pm_i$ ($i=1,\dots,N_{par}$),
where $N_{par}$ is the number of parameters. The central value $F_0$ and the uncertainty $\sigma^\pm_F$
on a PDF-dependent quantity are given by:
\begin{equation}
	\begin{split}
	F_0 &= F(S_0),\quad F_i^\pm=F(S_i^\pm) \\
	\sigma^+_F &= \sqrt{\sum_{i=1}^{N_{par}} \left[max\left(F_i^+ - F_0, F_0 - F^-_i,0\right)\right]^2 }\\
	\sigma^-_F &= \sqrt{\sum_{i=1}^{N_{par}} \left[max\left(F_0 - F^+_i, F_i^- - F_0,0\right)\right]^2 }
	\end{split}
\end{equation}

Whereas the uncertainty for NNPDF4.0 is calculated using the replicas method. Given $N_{rep}$ replica
PDF members $S^k$, the central value and the uncertainty is given by the mean and standard distribution
\begin{equation}
	\begin{split}
F_0&=\expval{F}=\frac{1}{N_{rep}}\sum_{i=1}^{N_{rep}}F\left(S^k\right)\\
\sigma_F&= \sqrt{\frac{1}{N_{rep}-1}\sum_{i=1}^{N_{rep}}\left[F\left(S^k\right)-F_0\right]^2}
	\end{split}
\end{equation}

...
It was later decided that these values of $p_1$ are not suitable for SeaQuest due the the lower
...
Each event is assigned a weight based on the cross section. The cross section is calculated based on
the kinematics of the generated events. The leading order cross section is calculated with the
CT14 pdf. The NLO effect is then included using 2-dimensional K-Factor \cite{shivangi-1747},
calculated using DYNNLO (see \cref{sec:DYNNLO}).
The proton on deuterium cross section is calculated using the charge symmetry, and assuming negligible nuclear correction.
...
Once the hits are identified, the Kalman track fitting can begin \cite{kalman1960}. The Kalman filter
parametrize a track based on the five parameters (the momentum, x-, y-slope, x- and y-position),
as well as the current $z$ value being evaluated. 
The Kalman filter then imperatively refines the these parameters by comparing the the measured hit location
to the prediction. At each iteration, the Kalman filter preform three steps: predict, filter, and smoothing.

During the predict and filter steps, the Kalman filter loops over each of the hits and calculate the
predicted hit position using the current estimate of the track parameters. 

The smoothing step propagates the changes in each node to the adjacent nodes. 
...
region. The energy loss and multiple scattering in the FMag are taken into account during this step.
...
\pdfcomment{kalment filter, refitting if it the pair originate from target to account for multiple scattering }
...
for these information. While most variables have consistent definitions across literature, the momentum
...
A minor beneficent is that this method also ensures the result to be reproducible.
...
the probability is relatively small. It is more likely to have multiple pions.
...
parmeters used $1.735$ for $J/\psi$ and $1.620$ for $\psi'$, and are consistent with the result in

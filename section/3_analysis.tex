...
section. This approach approach ensures high statistical precision in determining the acceptance 
at high mass and $x_F$, while also preserving the expected dimuon kinematic distribution.

For each dimuon, a mass and $x_F$ pair is chosen, the transverse momentum is also chosen. 
...
decays. These values are inherited from the previous Drell-Yan experiment at Fermilab.
It was latter decided that these values of $p_1$ are not suitable for SeaQuest due the the lower
beam energy, and the $P_T$ distribution is adjusted following a re-weighting procedure, which would
be described in \cref{subsec:pt_reweight}.
...
a $1+\cos^2\theta$ distribution with a uniform azimuthal angular distribution in $\phi$.
...
the kinematics of the generated events. The leading order cross section is calculated using 
CT14 pdf. The NLO effect is then included using K-Factor.

As for the charmonium decay, the event weight is based on the calculation using color evaporation
model, with the normalization taken from a particularization by E789 \cite{schub1995}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.5\linewidth]{pT_reweight/LD2_DY_p1}
...
\pdfcomment{the scale dependence}

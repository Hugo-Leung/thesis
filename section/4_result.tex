...
To understand the systematic uncertainties in the intensity extrapolation method due to the choice of 
fit function, the following functions are used.
\begin{align}
	R_i\left(I\right) &= p_{0i} + p_{1} I + p_{2} I^2\\
	R_i\left(I\right) &= p_{0i} + \left[p_{10} + p_{11}x_i\right] I + \left[p_{20} + p_{21}x_i\right]I^2\\
	R_i\left(I\right) &= p_{0i}
\end{align}

\pdfcomment{how the chisq compares between different fit functions and what does the result looks like}
...
\end{figure}
The charmonium ratios are closer to one as compared to the Drell-Yan cross section ratio. This is partly 
due to the contribution from gluon fusion process in the charmonium production. Assuming isospin symmetry,
the gluon distribution in the proton and neutron should be identical and therefore bring the charmonium ratio
closer to unity. While the quark-antiquark annihilation process in the charmonium production can be sensitive
to the light sea-quark asymmetry, similar to the Drell-Yan process, the charmonium production is insensitive
to the charge of the quarks and therefore larger contribution from the valence quark ratio, $d(x)/u(x)$.
The overall trends for the three process are in agreement with theoretical expectation. 

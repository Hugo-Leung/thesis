...
composite particles \cite{rabi1934a}. The modern value of the $g$ factors
...
\begin{table}[h!]
\centering
\caption{$g$ factor for $e$, $\mu$, $p$ and $n$ \cite{ParticleDataGroup:2022pth}.}
\label{tab:g-factor}
...
of the charge and magnetic moment distribution of the nucleon, and the mean square 
charge radius can be defined from the first derivative of the electric form factor.
\begin{equation}
	-6G^\prime_E(0) \equiv \expval{r^2}.
\end{equation}
This interpenetration is however later questioned in Ref.~\cite{miller2007,miller2019},
...
\Cref{fig:charge}
shows recent extraction of the proton and neutron transverse charge density from the 
...
In an attempt to understand the growing numbers discovered in the 1940s and 1950s,
...
and the wavefunction is symmetry in both flavor and spin, the magnetic moments are now given by
\begin{align}
...
\num{0.6849793(3)}\cite{ParticleDataGroup:2022pth}.
\begin{figure}[h!]
	\centering
	...
	\end{subfigure}
	\caption{Baryons and meson organized based on their charge(diagonal) and strangeness(vertical). }
	\label{fig:Octet}
\end{figure}
...
\end{figure}
The Gottfired Sum Rule introduced in \cref{sec:sum_rules} would also be modified due to
QCD effects. At order $\alpha_s^2$, in the limit of a symmetric sea, the Gottfired Sum Rule
...
instead of only the integral as with the Gottfired sum rule.
...
\begin{equation}
	\ket{p} = \sqrt{Z}\ket{p}_{bare} + \sum_{MB}\int \dd{y} \dd[2]{\vec{k}_\perp} \phi_{BM} \left(y, k^2_\perp\right)\ket{B\left(y, \vec{k}_\perp\right);M\left(1-y, -\vec{k}_\perp\right)}
\end{equation}
...
$\vec{k}_\perp$ and $-\vec{k}_\perp$ respectively. $Z$ is the standard wave function renormalization factor 
...
quark distribution in pions and $S(x)$ is the flavor symmetric contributions. If we took the difference 
between the two distribution, the flavor symmetric contributions $S(x)$ would cancel out.
...
In this model, the potential of a quark $q_i^h$ with helicity $h$ has a opposite sign to the that of the 

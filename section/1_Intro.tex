...
both composite particles~\cite{rabi1934}. The modern value of the $g$ factors
...
\begin{equation}
	F_2\left(x,Q^2\right)=x\sum_i e^2_i f_i/h\left(x,Q^2\right),
	\label{eq:F2_parton}
\end{equation}
...
by using the isospin symmetry to relate the neutron distributions to those of
...
\end{equation}
The first approximation sign indicate that nuclear correction in deuteron is assumed to be negligible.
...
based on BHPS model, with normalization fixed by the measured asymmetry.
\begin{figure}
	\centering
...
development in understand the nonperturbative charm in the proton~\cite{guzzi2023}. There are
also suggestion that SeaQuest kinematic could be ideal for testing limits on intrinsic charm~\cite{vogt2021}.
...
\pdfmargincomment{Charge symmetry or isospin symmetry? I used isospin in the rest of the thesis}
...
At our beam energy, the quark-antiquark annihilation bcomes the dominant subprocess at large $x_F$, where

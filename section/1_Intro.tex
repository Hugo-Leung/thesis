...
\label{ch:intro}
This paper is organized as follows. After reviewing the early studies
...
\Cref{M-ch:analysis} covers the procedure for data analysis. The results are 
presented in \cref{M-ch:result}, followed by a summary and future prospects
...
\begin{equation}
	...
	\label{eq:DY_cs}
\end{equation}
\Cref{fig:scaling} shows some of the existing data on DIS and Drell-Yan. By
...
\end{subfigure}
\caption{The calculated isovector PDF from the LP\textsubscript{3} collaboration (left)
\cite{alexandrou2018}(right)and a Cyprus group (right)\cite{alexandrou2018}(right).
...
\chapter{Charmonium Production and Gluon Distribution}
\label{ch:jpsi}

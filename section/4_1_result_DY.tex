...
\end{figure}
With the yield for different process obtained from the mass fits, 
the cross section ratio as a function of $x_T$ can be calculated
...
\begin{figure}[h!]
	...
	\caption{The extracted Drell-Yan cross section ratio as a function of $x_T$
		using the mass fit method from Run 2-3 (black square) and Run 5-6 (blue triangle),
		as well as the combined result (red circle). The curve corresponds to calculations
...
The red points in \cref{fig:CSR_MF_xT} results from the combined analysis following the procedure in \cref{M-subsec:combine}.
\FloatBarrier
...
The updated corrections are used through out this thesis.
\begin{figure}[h!]
	\centering
...
\subsubsection{Cross section ratio vs other variable}
Following similar procedure, the Drell-Yan cross section ratio as a function of $x_B$ (\cref{fig:CSR_MF_xB})
and $x_F$ (\cref{fig:CSR_MF_xF}) are also extracted.
\begin{figure}[h!]
	\centering
...
In particular, \cref{fig:CSR_MF_xT} shows the calculation using CT18 (without the SeaQuest data)
...
calculation, this is partly due to the updated contamination correction.
The impact of the SeaQuest measurement is further illustrated in \cref{fig:JAM_impact}. 
The JAM analysis~\cite{cocuzza2021} found that the SeaQuest measurement greatly reduce the 
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.5\linewidth]{JAM_impact.png}
	\caption{impact on the $\bar{d}/\bar{u}$ ratio (top) and the asymmetry $x\left(\bar{d}-\bar{u}\right)$
		of the SeaQuest measurement (red band) and STAR $W$ data \cite{adam2021} (blue band) relative to the baseline (green band)
		in the JAM analysis~\cite{cocuzza2021}. Taken from Ref.~\cite{cocuzza2021}.}
...
\end{align}
The fit to data using the two fits selected $x_T$ bins are shown in \cref{fig:FIT1_selected,fig:FIT2_selected}
and $\chi^2$ for the fits are tabulated in \cref{tab:chi_IE}.
And the fit in other $x_T$ bins and for other variables are shown in \cref{M-a_ch:extrapolation}.
...
While the difference caused by the fit functions are small for $x_T$ in Run 2-3, as reported in Ref.~\cite{dove2021},
...
\end{figure}
The results from the two method mostly agree with each other,
...
The final cross section ratio is obtained by taking the average of the two method and are shown in red in the \cref{fig:CSR_method}.

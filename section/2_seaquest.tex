...
dump and hadron absorber. As a focusing magnet, FMag would focuses dimuons with high $p_T$
or high mass into the spectrometer acceptance. This is particularly important as SeaQuest
aims to probe the partonic structure at high $x$, which would corresponds to high mass.
...
and therefore allowing muons into the spectrometer.
...
of the beam with the beam dump further away from the targets, allows for better target dump
separation. This also reduces the radiation within the target cave, which facilitate the
access to the target for maintenance.
The magnetic field distribution is modeled using a magnetostatic modeling program. The
excitation measured by wrapping a 1-turn coil around the central region of the magnet and measuring the induced
current. The final calibration of the field strength is determined by the reconstructed
mass of the $J/\psi$ resonance.

The downstream analyzing magent (KMag) is a \SI{3}{\meter} open aperture magnet measures at
\SI{300}{\cm} long with a \SI{289}{\cm} wide by \SI{203}{\cm} high central air gap. It was
...
A hodoscope is place on upstream end of each tracking. Each of the planes
is composed of overlapping panels of scintillators which a PMT at the end.
When a charged particle pass through the scintillator, the molecules would be excited and
release photons. These photons are then collected at the PMT, which is then read out by the
by the Time-to-Digital Converter (TDC).
...
The scintillator bars in each plane is also splitted in the middle, denoted at Top and Bottom
...
cards which are used to amplification and discrimination. The signal is then fed to LSB (Level Shifter Board)
...
the ``Event DAQ'', ``Scaler DAQ'' and ``Beam DAQ''. The slow control system

\subsubsection{Event DAQ}
The Event DAQ records the event-by-event hit information and the trigger timing.
It use the VME-based ``CODA'' \cite{CODA} system developed by Thomas Jefferson National Accelerator
Facility. This system uses 14 VME crates and can read out different detectors in parallel.
Each VME crate contains a board processor, a trigger interface and a number TDCs
to be readout. Each TDC will read out the hit information in each
There is also a trigger supervisor crate.

The trigger supervisor receive the trigger single from the trigger matrix and NIM triggers. The trigger supervisor
...
When the trigger supervisor receive a trigger, it generate a common stop signal, which is fan out
to the trigger interface in each VME crate. The trigger interface then instruct the processor to readout the TDC
and send the hit information back to the DAQ host over gigabit Ethernet. The trigger interface then
reports back the the trigger supervisor once the crate has completed the readout. The trigger supervisor can process
...
dominated by the time required to readout all the TDC in a single crate. Even though
each detector channels is readout in parallel, the TDCs are readout in sequentially
...
a common-stop signal is issued to all TDC. The TDCs will then store the hit information
...
cards. The Scaler DAQ is triggered by the a \SI{7.5}{\kilo\hertz} clock and the beam
spill signal.
...
System (EPICS)\cite{dalesio1994}, to synchronize the different DAQ data stream, monitor the various system.
...
This allows us to the record the current to the different spectrometer magnets, target position 
...
Typically, each run of physical data taking last about an hour. The file size is typically
several \unit{\giga\byte}. The data files from all the DAQ systems and slow control system
are stored locally in a disk array located in the SeaQuest control room. The data from DAQ 
is also backed up on tape storage provided by Fermilab computing sicision.
...
These raw data files are process by a custom program called ``decoder", which coverts the 
CODA specific format in a ROOT based format. These information will also be uploaded to a SQL 
server.

...
\begin{figure}[htpb!]
	...
	\caption{The beam intensity measured by the Beam DAQ Cherenkov counter every
...
\begin{figure}[htbp!]
	...
	\caption{The Beam Intensity Monitor (BIM) Cherenkov  counter. Taken from Ref.\
		\cite{aidala2019}.}
	\label{fig:BIM}
\end{figure}
...
Downstream of station 3, there is an \SI{1}{\meter} iron wall acting as a
...
separation. This also reduces the radiation within the target cave, which facilitate 
...
When a charged particle passes through the scintillator, the molecules would be excited and
release photons. These photons are then collected by the PMTs, the amplified signals are
...
is used for St.~3 and 4. Because of the physical size of the sintillatior bars in St.~4, the St.~4
...
in each module are further divided into two sub-layers, with the second layers shifted by half a tube width to
...
The Beam DAQ aims to provide the instantaneous beam intensity with the Cherenkov  counter.
The signal from the Cherenkov  counter is sent to a Fermilab-designed QIE module.
This custom module is synced to Main Injector RF clock and it reads out the Cherenkov
